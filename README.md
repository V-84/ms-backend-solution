...
The URI for this test stub in Mocky is https://run.mocky.io/v3/62aa0630-5aaa-4665-bc71-875941d6f482.
...
The URI for this test stub in Mocky is https://run.mocky.io/v3/cda925b1-c3d9-4c2e-af2e-d65f45310927.
...
1. I Isolated the business logic from route handling into it's own service files. This meant changes will affect only specific domains. The benefits of this is cleaner route files and separation of concerns

2. I refactored the api response format to this. This ensures a predictable API contract which makes it easy to add metadata (e.g pagination) if needed. It alse simplifies response handling e.g `if (response.status === 'success') `
```
{
...
3. In this basic implementation for failover, the success/failure metrics are not atomic. Meaning it might not behave the exact way in should in a highly concurrent and multi-node cluster environment.

4. Failover Mechanism in a multi-node cluster.: In this mode, I will go with a distributed caching solution like Redis.

5. Error Handling could have been done better, creating a reusable error handler.
   
6. For the provider logger, I could have created another table called providers and use the id as the foreign key on the provider-logs table. That way, if there are any provider details that needs to be stored, we will do that in the provider table instead of the provider-logs table.